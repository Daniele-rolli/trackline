...
const SETUP_AUTH_REQUIRED_VALUE =
  SETUP_AUTH_REQUIRED_RAW && SETUP_AUTH_REQUIRED_RAW.trim().length > 0
...
const REQUIRE_HTTPS_VALUE =
  REQUIRE_HTTPS_RAW && REQUIRE_HTTPS_RAW.trim().length > 0
    ? REQUIRE_HTTPS_RAW
    : HOST === "127.0.0.1"
      ? "false"
      : "true";
...
function resolvePrivateKeyConfig(): PrivateKeyConfig {
...
function missingForDevToken(): string[] {
...
function missingForPolling(): string[] {
...
function setupErrorMessage(missing: readonly string[]): string {
...
function requestProto(c: Context): string {
...
function requireAllowedHost(c: Context): Response | null {
...
function requireHttps(c: Context): Response | null {
  if (!REQUIRE_HTTPS) {
    return null;
  }
...
function requireSetupEnabled(c: Context): Response | null {
  if (SETUP_ENABLED) {
    return null;
  }

  return c.json(
    { error: "Setup routes are disabled. Set SETUP_ENABLED=true to enable onboarding endpoints." },
    404
  );
}

function normalizeLimit(input: number, fallback: number): number {
...
function enforceRateLimit(c: Context, bucket: string, limitInput: number): Response | null {
...
function requireSetupAuth(c: Context): Response | null {
  const host = requestHost(c);
  ...
  if ((SETUP_AUTH_REQUIRED || publicRequestWithoutToken) && !SETUP_AUTH_TOKEN.trim()) {
    return c.json(
      {
        error:
          "SETUP_AUTH_TOKEN is required for setup routes in this deployment mode. Configure it in runtime/.env.",
      },
      403
    );
  }
  ...
  return c.json(
    {
      error:
        "Unauthorized. Provide setup token using Authorization: Bearer <token> or ?token=<token>.",
    },
    401
  );
}

function requireNowPlayingAuth(c: Context): Response | null {
  ...
  return c.json(
    {
      error:
        "Unauthorized. Provide now-playing token using Authorization: Bearer <token> or ?token=<token>.",
    },
    401
  );
}
...
interface PlaybackHeuristicState {
  ...
  currentTrackTransitionAt: number | null;
}
...
function artUrl(raw: string, size = 300): string {
...
async function poll(): Promise<void> {
  ...
  try {
    ...
    const now = Date.now();
    const trackId = String((item as { id?: unknown }).id ?? "");
